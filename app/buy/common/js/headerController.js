...
//var deviceIdOrigin;
'use strict';
angular.module('headerApp', ['LocalStorageModule'])
	.controller('headerController', function ($rootScope, $scope, $http, localStorageService, $anchorScroll, RestAPI, $location, $sce, $window, $timeout) {
		...
		$scope.callToFetch = function () {
			...
			if (wordPressEnabled) {
				...
				setTimeout(function () {
					if (localStorageService.get("userProfileDetails")) {
						...
						$scope.userLoginInfo = localStorageService.get("userProfileDetails");
						if ($scope.userLoginInfo.firstName) {
							$('.signin_link').text($scope.userLoginInfo.firstName+" Account");
			...
			$scope.init = function () {
				...
				$scope.callQuoteController = function (tab) {
					$anchorScroll('home');
					if (tab.businessLineId != localStorageService.get("selectedBusinessLineId")) {
				...
				$rootScope.showFeedbackForm = function (feedType) {
					...
					//feedbackForm]
					$scope.feedback = [{ 'name': '', 'mobileNumber': '', 'emailId': '', 'description': '' }];
					if (feedType == 'userLikes') {
						$scope.docType = "What you liked";
						$scope.placeholder = "for e.g. I like the way website is very sleek and easy to navigate."
					} else {
						$scope.docType = "What can be better";
						$scope.placeholder = "for e.g. It was not evident that i am on Policy Quote page of the website. I prefer a heading or page title bar in each page would help."
					}
				...
				$scope.sendEmail = function (contactUsForm) {
					if (contactUsForm.$invalid) {
						...
						$scope.inputValidStatus = false;
						$scope.composeEmail.paramMap.userEmail = $scope.composeEmail.userEmail;
						...
						quoteUserInfo.mobileNumber = $scope.composeEmail.paramMap.mobileNumber;
						...
						if (urlPattern != "/life" && urlPattern != "/car" && urlPattern != "/health" && urlPattern.indexOf("pay") == -1 && urlPattern.indexOf("proposalresdata") == -1 && urlPattern.indexOf("sharefromAPI") == -1 && urlPattern.indexOf("ipos") == -1) {
							userInfoWithQuoteParam.requestSource = sourceOrigin;
						}

						$scope.composeEmail.userInfoWithQuoteParam = userInfoWithQuoteParam;
				...
				$scope.getCampaignFunction = function () {
					...
					if ($location.search().isForRenewal && $location.search().campaign) {
			...
			if (p365cookie) {
				...
				if (agencyPortalEnabled) {
					if ($location.search().P365Token) {
						...
						RestAPI.invoke("verifyToken", verifyTokenReq).then(function (tokenVerificationCallback) {
							if (tokenVerificationCallback.responseCode == 1000) {
								$rootScope.accessDenied = false;
								tokenVerificationResp = tokenVerificationCallback.data;
								if (tokenVerificationResp && tokenVerificationResp.issuer == "Policies365") {
									RestAPI.invoke("validateSource", request).then(function (callback) {
										if (callback.responseCode == 1000) {
											$scope.sourceValidationResponse = callback.data;
											sourceOrigin = $scope.sourceValidationResponse.source;
											deviceIdOrigin = $scope.sourceValidationResponse.deviceId;
											requestSource = $scope.sourceValidationResponse.deviceId;
											$rootScope.affilateURL=$scope.sourceValidationResponse.affilateURL;
											$rootScope.affilateUser = $scope.sourceValidationResponse.affilateUser;
											$rootScope.affilateRedirection = $scope.sourceValidationResponse.redirectToAffilateScreen;
											campaign_id = $scope.sourceValidationResponse.campaignId;
											if($scope.sourceValidationResponse.hideLOB){
